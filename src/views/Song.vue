<template>
  <main>
    ...
    <section class="w-full mb-8 py-14 text-center text-white relative">
      <div class="absolute bg-cover lg:bg-contain inset-0 w-full h-full box-border music-bg"
...
export default {
  ...
  methods: {
    ...
    async addComment(values, { resetForm }) {
      ...
      this.comment_alert_message = 'Your comment has been submitted! Thanks for your feedback.';

      resetForm();
    },
    async getComments() {
      const commentArr = await commentsCollection.where(
        'sid', '==', this.$route.params.id,
      ).get();

      this.comments = [];

      commentArr.forEach((doc) => [
        this.comments.push({
          docID: doc.id,
          ...doc.data(),
        }),
